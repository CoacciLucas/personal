# Personal - Research Document

## Overview

**Personal** is a SaaS (Software as a Service) product developed by **Lucas Montano**, designed to help users improve their persuasion and communication skills in real-time during video calls and presentations.

- **Website**: [https://personal.com](https://personal.com)
- **Tagline**: "Unleash your persuasion superpower in real-time"
- **Creator**: Lucas Montano ([@lucas_montano](https://twitter.com/lucas_montano))

---

## Company Information

| Field | Details |
|-------|---------|
| **Company Name** | Montano Productions B.V. |
| **Location** | John M. Keynesplein 1, 1066 EP Amsterdam, Netherlands |
| **KvK Number** | 93885067 |
| **Support Email** | help@personal.com |
| **Legal Email** | legal@personal.com |
| **Governing Law** | Netherlands (Disputes resolved in Amsterdam courts) |

---

## Product Description

Personal is a **desktop application** that uses AI to analyze your system audio and screen context to provide real-time insights and suggestions during video calls and presentations.

### How It Works
1. The application connects with AI services (OpenAI and Google Gemini) using your own API key
2. It listens to your conversations and analyzes screen content
3. Provides real-time suggestions, facts, arguments, and answers to difficult questions
4. Operates discreetly in the background without appearing in screen shares

---

## Key Features

### 1. Real-Time Guide
- Listens to your conversations while you speak
- Suggests facts, arguments, and answers to difficult questions in real-time
- Helps you maintain confidence throughout your presentation

### 2. Objection Management
- Provides persuasive and contextualized replies
- Helps maintain control of conversations
- Enables closing with authority

### 3. Instant Integration
- Works directly with major video conferencing platforms:
  - **Zoom**
  - **Google Meet**
  - **Microsoft Teams**
  - **Google Hangouts**
- No complicated installations required
- Invisible during screen sharing

### 4. Discreet Operation
- Works in the background
- Completely invisible during screen sharing
- Doesn't appear in broadcasts or display visible indicators
- Analyzes audio and video without the audience noticing
- Ensures distraction-free presentations

---

## Target Audience

### Sales Professionals
Close more deals by responding to objections with data and confidence.

### Entrepreneurs
Present your vision convincingly and win over investors and customers.

### Executives & Directors
Conduct strategic meetings with instant data at your fingertips.

### Interviewers
Formulate impactful questions and react to unexpected answers with real-time follow-up suggestions.

### Any Presenter
From interviews to conferences, elevate your performance.

---

## Benefits

| Benefit | Description |
|---------|-------------|
| **Unbeatable Confidence** | Never be without an answer, speak with total confidence |
| **Maximum Productivity** | Save hours by automating follow-ups and responses |
| **Competitive Advantage** | Outperform competitors with flawless, AI-powered presentations |
| **Continuous Improvement** | Analyze your performance and improve with each presentation |

---

## Pricing Model

Personal offers two types of plans:

### Free Plan
- Limited functionality
- Basic features included

### Paid Subscription Plans
- Unlock additional features and capabilities
- Recurring billing (monthly/annual)
- Prices subject to change with notice

### Billing Information
- Payment method charged on a recurring basis
- Can cancel anytime via account settings
- Access to paid features remains active until end of billing cycle
- No prorated refunds offered

---

## Technical Requirements

- **Platform**: Desktop application
- **API Key Required**: Users must supply their own API key for AI services
- **Supported AI Services**: 
  - OpenAI
  - Google Gemini
- **Permissions Required**:
  - System audio access
  - Screen capture/screen context access

---

## Data Collection & Privacy

### Personal Information Collected
- Contact details (name, email address)
- Account credentials (username and password)
- Audio and screen data (when permissions are granted)
- Billing information (for paid subscriptions)
- Communications (support inquiries)

### How Data Is Used
- Provide, personalize, and improve the Services
- Deliver contextual AI-generated insights
- Handle billing and manage subscriptions
- Offer customer support
- Analyze usage for research and development
- Detect fraud and ensure security

### Data Sharing
- Service providers (hosting, analytics, payments)
- Legal requirements when necessary
- Business transfers (mergers, acquisitions)

### Important Privacy Notes
- Users responsible for informing all parties about audio capture
- Users must obtain legally required consent before enabling audio processing
- Not intended for users under 13 years old
- Data may be transferred to EU servers

---

## Acceptable Use Policy

Users agree NOT to:
- Use the Services to break laws or third-party rights
- Record conversations in jurisdictions requiring consent without obtaining it
- Attempt to interfere with or degrade the system's performance
- Automate access in any unauthorized manner

**Important**: Use must remain ethical and non-deceptive in all contexts, including interviews, meetings, and educational assessments.

---

## Screen-Sharing Detection Disclaimer

> Personal is designed to operate discreetly during video calls and screen-sharing sessions. However, due to the evolving nature of detection systems and security software, we cannot guarantee that the application will remain completely invisible in all contexts. 

By using Personal, you acknowledge and accept that:
- Certain platforms, organizations, or security systems may detect or display the presence of the software
- The company is not liable for any consequences—including disciplinary, contractual, or reputational—that arise from such detection or visibility

---

## Warranty & Liability

### Warranty Disclaimer
- Services provided "AS IS"
- No guarantees on results
- AI-generated responses may be inaccurate, biased, or misleading
- Users should not rely on outputs without verification

### Limitation of Liability
- Company not liable for indirect, incidental, or consequential damages
- Total liability capped at amount paid in the 6 months prior to incident

---

## User Responsibilities

1. **API Key Ownership**: Users supply and maintain their own API key
2. **Charges**: Users responsible for any charges from third-party AI providers
3. **Legal Compliance**: Users must comply with applicable laws, including privacy and recording regulations
4. **Verification**: Users must independently verify any critical outputs

---

## Cancellation & Termination

### User Cancellation
- Can cancel subscription at any time via account settings
- Access to paid features remains until end of billing cycle
- Can also cancel by uninstalling the application

### Company Termination
- Company may terminate or suspend access at any time for any reason
- Upon termination, license revoked and data may be deleted

---

## Contact Information

| Purpose | Contact |
|---------|---------|
| **General Support** | help@personal.com |
| **Legal Inquiries** | legal@personal.com |
| **Address** | Montano Productions B.V., John M. Keynesplein 1, 1066 EP Amsterdam |

---

## Social Media

- **Twitter**: [@lucas_montano](https://twitter.com/lucas_montano)

---

## Document Information

| Field | Value |
|-------|-------|
| **Research Date** | March 2026 |
| **Source** | https://personal.com |
| **Terms Last Updated** | May 12, 2025 |
| **Privacy Policy Last Updated** | May 12, 2025 |

---

## Summary

Personal is an innovative AI-powered desktop application created by Lucas Montano that helps professionals improve their communication and persuasion skills during video calls and presentations. It integrates seamlessly with popular video conferencing platforms and provides real-time suggestions, objection handling, and contextual insights using OpenAI and Google Gemini APIs.

The product is particularly useful for sales professionals, entrepreneurs, executives, interviewers, and anyone who regularly conducts presentations or meetings online. It operates discreetly in the background, providing assistance without being visible to other participants.

The company operates under Dutch law as Montano Productions B.V., based in Amsterdam, and offers both free and paid subscription plans.
